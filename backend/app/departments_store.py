...
class DepartmentsStore:
    def __init__(self) -> None:
        ...
        self._file_path = self._data_dir / "departments.json"
        self._seed_data = [
            {"id": 1, "code": "FRAME", "name": "Frame Manufacturing", "is_active": True},
            {"id": 2, "code": "LENS", "name": "Lens Processing", "is_active": True},
            {"id": 3, "code": "QC", "name": "Quality Control", "is_active": True},
        ]
    ...
    def upsert(self, payload: dict[str, Any]) -> tuple[dict[str, Any], bool]:
        ...
        updated = self.update_by_code(
            str(payload["code"]),
            {"name": payload["name"], "is_active": payload.get("is_active", True)},
        )
        if updated is None:
            return self.create(payload), True
